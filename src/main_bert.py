from helpers import clean_text, analyze_sentiment_bert
...
grouped_df = df.groupby("Product_id").agg(
    {"Sentiment": ["count", lambda x: (x == "Positive").mean()]}
)
...
positive_recommendations = df.groupby("Product_id").filter(
    lambda x: (x["Sentiment"] == "Positive").mean() > 0.8
)
...
negative_recommendations = df.groupby("Product_id").filter(
    lambda x: (x["Sentiment"] == "Negative").mean() > 0.8
)
